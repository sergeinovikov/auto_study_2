...
Функция: API Кейс 1. Создание, изменение, получение, удаление пользователя. Администратор системы
  ...
  Сценарий: Создание, изменение, получение, удаление пользователя. Администратор системы
    ...
    Если Изменить у пользователя "Пользователь" изменить почтовый адрес на невалидный, а пароль - на строку из 4 символов и назначить данные пользователю "Невалидный пользователь"
    И Создать POST-запрос на создание пользователя "Невалидный пользователь" с запросом "Запрос на создание пользователя c невалидными данными"
    ...
    И Информация в базе данных об изменённом пользователе "Пользователь 2" присутствует, статус = 1
